              ...
              <>
                  ...
                  {entries.map((entry, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-sm text-muted-foreground">
                          {Math.round(food.calories_kcal * factor)} kcal ·{' '}
                          {(food.carbs_g * factor).toFixed(1)}g carbs ·{' '}
                          {(food.protein_g * factor).toFixed(1)}g protein ·{' '}
                          {(food.fat_g * factor).toFixed(1)}g fat
                        </div>
                      </div>
                    )
                  })}
                ...
                <div className="pt-4 border-t space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-1 text-sm">
                    <div>Calories: {Math.round(mealTotals.calories_kcal)} kcal</div>
                    <div>Carbs: {mealTotals.carbs_g.toFixed(1)}g</div>
                    <div>Protein: {mealTotals.protein_g.toFixed(1)}g</div>
                    <div>Fat: {mealTotals.fat_g.toFixed(1)}g</div>
                    <div>Potassium: {Math.round(mealTotals.potassium_mg)}mg</div>
                    <div>Sodium: {Math.round(mealTotals.sodium_mg)}mg</div>
                  </div>
                </div>
              </>
            )}
    ...
    <CardContent>
      <div className="grid grid-cols-2 gap-x-4 gap-y-4 text-sm">
        <div>
          ...
          <div className="font-medium">{Math.round(dailyTotals.calories_kcal)} kcal</div>
        </div>
        <div>
          <div className="text-muted-foreground">Carbs</div>
          <div className="font-medium">{dailyTotals.carbs_g.toFixed(1)}g</div>
        </div>
        <div>
          <div className="text-muted-foreground">Protein</div>
          <div className="font-medium">{dailyTotals.protein_g.toFixed(1)}g</div>
        </div>
        <div>
          <div className="text-muted-foreground">Fat</div>
          <div className="font-medium">{dailyTotals.fat_g.toFixed(1)}g</div>
        </div>
        <div>
          <div className="text-muted-foreground">Potassium</div>
          <div className="font-medium">{Math.round(dailyTotals.potassium_mg)} mg</div>
        </div>
        <div>
          <div className="text-muted-foreground">Sodium</div>
          <div className="font-medium">{Math.round(dailyTotals.sodium_mg)} mg</div>
        </div>
        <div>
          <div className="text-muted-foreground">Phosphorus</div>
          <div className="font-medium">{Math.round(dailyTotals.phosphorus_mg)} mg</div>
        </div>
        <div>
          <div className="text-muted-foreground">Fiber</div>
          <div className="font-medium">{dailyTotals.fiber_g.toFixed(1)}g</div>
        </div>
      </div>
    </CardContent>
  </Card>
    ...
    <CardContent>
      {constraintViolations.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-4 text-center">
          ...
          <p className="text-green-700 font-medium">All constraints met</p>
        </div>
      ) : (
          ...
          {constraintViolations.map((v, idx) => (
            ...
            >
              ...
              <div className="text-sm">
                ...
                {v.mealName && (
                  <div className="text-xs text-muted-foreground">{v.mealName}</div>
                )}
              </div>
            </div>
          ))}
